$(document).ready(function(){
    ...
    //rounding a function if an answer includes a decimal
    function round(val){
    ...
    $('button').click(function(){
        ...
        //reseting log ater answer to equation
        if (reset){
        ...
        //to prevent more than one decimal in a number
        if(entry === '.' || entry === '0.'){
            if(!decimal){
                entry = '';
            }
        ...
        //prevent the improper use of first digit
        if(ans.length === 0 && isNaN(entry) && entry !== '.' || ans.length === 0 && entry === '0'){
            entry = '';
            ans = '';
        }

        //to prevent extra operators
        if(current !== 'noChange'){
        ...
        //to prevent digit combining
        while (Number(entry) || entry === '0' || current ==='.'){
            if(isNaN(current) && entry === '0' && current !== '0'){
                entry = '';
        ...
        console.log($('#history').tex().length);
    });
});